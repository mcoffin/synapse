...
class Synapse::ServiceWatcher
  class MarathonCniWatcher < BaseWatcher
    def start
    ...
    def watch
      until @should_exit
        ...
        begin
          ...
          creds = @discovery['marathon_api_credentials']
          ...
          backends = tasks.keep_if { |task| task['state'] == 'TASK_RUNNING' }.map do |task|
            {
              'name' => task['id'],
              'host' => task['ipAddresses'][0]['ipAddress'],
              'port' => @discovery['container_port'] || task['ports'][port_index],
            }
